        ...
        if ($insert_macros)
        {
            foreach ($insert_macros as $insert_macro)
            {
                ...
                if ($symbol != null)
                {
                    ...
                    start_output_buffer();
                    $this->loadTemplateFile('view.macro', array('symbol' => $symbol, 'type' => $type, 'advance' => $advance, 'timeframe' => $timeframe, 'title' => $title, 'hover' => $hover), false);
